...
Because we work with random numbers, the accuracy can sometimes be better for a smaller $N$ than for a large $N$ for statistical reasons: Therefore, in Python-Code \ref{py:code} we used the mean of ten $\pi$ calculations to determine the accuracy for a given $N$.
Regardless of the statistical fluctuations, the relative error in the $\log$-$\log$-plot decreases linearly (see Fig. \ref{fig:accuracy}).\\
Using the rejection method, we get (for example) the following value for $\pi$:
\begin{equation}
	\boxed{\:\text{Rejection Method with $N=6\cdot 10^7$:}\qquad\quad
		\pi\approx\num{3.141445}\:}
\end{equation}
...
We know that the integral of $f(x)=\sqrt{1-x^2}$ on the interval $0\leq x\leq 1$ gives a quarter of the area of the unit circle.\\
The area of the unit circle (radius $r=1$) is $A_\mathrm{c}=\pi r^2=\pi$. With $N$ randomly selected values $x_i\in [0,\,1)$, we obtain the following approximation formula for $\pi$ using Monte Carlo integration:
\begin{equation}
