...
condition for fixed points (FP) in the case of interacting populations:
\begin{equation}
	\text{$n=n^*$ stationary point of the system}
	\qquad\quad\Longleftrightarrow\quad\qquad
	f(n^*) := \frac{\mathrm{d}n}{\mathrm{d}\tau}\Big\vert_{n=n^*} = 0
\end{equation}
stability analysis in the multi-dimensional case: analogous to one-dimensional, but vector form:
\begin{equation}
	\vec{f}(\vec{N},\,\vec{P}) =
	\begin{pmatrix}
		\dot{\vec{N}} \\
...
\begin{align}
	...
	= N_i\left(a_i-N_i-\sum_{j}b_{ij}P_j\right) \\
	p_i(P_i,\,\vec{N})
	&:= \frac{\mathrm{d}P_i}{\mathrm{d}t} 
	= P_i\left(\sum_{j}c_{ij}N_j-d_i\right)
\end{align}
...
\begin{figure}[t!]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Predator $P_3$ and prey $N_3$}
		\label{fig:species-3}
	\end{subfigure}
