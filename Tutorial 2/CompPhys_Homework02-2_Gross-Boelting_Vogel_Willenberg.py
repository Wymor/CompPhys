...
def relative_error(energy):
    ''' Function to compute the relative error in the total energy '''
...
dt = np.linspace(1e-3, 1e3, int(1e2))
...
for i in range(0,len(v0)):
    for j in range(0,len(dt)):
        ...
        s, w = two_body_problem(body1, body2, 1., dt[j], 2*np.sqrt(1.**3/(1.*2.))/dt[j])
        rel_error[i,j] = relative_error(total_energy(s,w))[int(1e3-1)]

# Plot the results
fig, ax = plt.subplots()
ax.plot(dt, rel_error[0,:], 'b.', label='Test')
ax.plot(dt, rel_error[1,:], 'r.', label='Test')
ax.set_title(r'Error Analysis of Euler Scheme')
ax.set_xlabel(r'step size $h$'); ax.set_ylabel(r'relative error $\epsilon$')
